...
import { Menu, X, Database, LogOut, FileText, UserCircle, Settings } from 'lucide-react';
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Author layout accessible to both AUTHOR and ADMIN
...
export default function AuthorLayout() {
  ...
  const navLinks = [
    ...
    { to: '/author-blogs', label: 'My Blogs', icon: FileText },
      ...
      <nav className="border-b border-border bg-card px-6 py-3 flex justify-between items-center z-10 sticky top-0">
        <div className="flex items-center space-x-4">
          ...
          <NavLink to="/" className="flex items-center space-x-2 cursor-pointer">
            ...
            <Heading as="span" variant="h3" className="tracking-tight">Author Studio</Heading>
            <span className="mono-text text-[9px] bg-blue-500/20 text-blue-500 border border-blue-500/30 px-1.5 py-0.5 rounded font-semibold uppercase">
              AUTHOR
            </span>
          </NavLink>
        </div>
