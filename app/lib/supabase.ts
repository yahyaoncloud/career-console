...
// Use VITE_ prefix for client, fall back to non-prefixed for server
const supabaseUrl  = (import.meta.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL) as string;
const supabaseAnon = (import.meta.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY) as string;

if (!supabaseUrl || !supabaseAnon) {
  console.warn('[Supabase] SUPABASE_URL or SUPABASE_ANON_KEY is not set. File uploads will be unavailable.');
}
